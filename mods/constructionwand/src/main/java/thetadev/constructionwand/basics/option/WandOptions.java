...
public class WandOptions
{
    ...
    public WandOptions(ItemStack wandStack) {
        stack = wandStack;
        rootTag = wandStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag();
        if(rootTag.contains(TAG_ROOT)) tag = rootTag.getCompound(TAG_ROOT);
        else {
            tag = new CompoundTag();
    ...
    public boolean addUpgrade(IWandUpgrade upgrade) {
        if(upgrade instanceof IWandCore) {
            boolean added = cores.addUpgrade((IWandCore) upgrade);
            if(added) writeToStack();
            return added;
        }
        return false;
    }
